...
async function loadStory(story) {
  const filePath = path.join(rootDir, "data", `${story}-sentences.json`);
...
async function main() {
  ...
  const story = getArg("story", "story1");
  ...
  const sentences = await loadStory(story);
  const outputDir = path.join(rootDir, "audio", story);
